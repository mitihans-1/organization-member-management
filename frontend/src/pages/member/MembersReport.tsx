...
const MembersReport: React.FC = () => {
  ...
  const { data: reportData, isLoading } = useQuery({
    ...
    queryFn: () => reportService.getMemberAnalytics({
      tab: activeReport,
      dateRange,
      search: searchQuery,
      ...
      tkStatus: ticketStatusFilter,
      tkPriority: ticketPriorityFilter,
      tkCategory: ticketCategoryFilter,
      tkAssigned: assignedToFilter,
      payStatus: paymentStatusFilter,
      payMethod: paymentMethodFilter,
      payType: paymentTypeFilter
    })
  });
  ...
  const getQuickStats = () => {
    if (activeReport === 'overview' && reportData?.quickStats) {
      const statsIcons = [Calendar, Briefcase, Ticket, CreditCard];
      ...
      return reportData.quickStats.map((stat: any, idx: number) => ({
        label: stat.label,
        value: stat.value,
        icon: statsIcons[idx] || Ticket,
        color: statsColors[idx] || 'text-rose-500',
        bg: statsBgs[idx] || 'bg-rose-50'
      }));
    }
    return [
      { label: 'Events Attended', value: '...', icon: Calendar, color: 'text-indigo-500', bg: 'bg-indigo-50' },
      { label: 'Services Requested', value: '...', icon: Briefcase, color: 'text-emerald-500', bg: 'bg-emerald-50' },
      { label: 'Tickets Submitted', value: '...', icon: Ticket, color: 'text-amber-500', bg: 'bg-amber-50' },
      { label: 'Total Paid', value: '...', icon: CreditCard, color: 'text-rose-500', bg: 'bg-rose-50' },
    ];
  };
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-4 flex gap-3 flex-wrap items-center">
        ...
        {hasActiveFilters && (
          <button
            ...
            className="flex items-center gap-1 px-3 py-2 text-sm text-gray-500 hover:text-gray-700 border border-gray-200 rounded-xl bg-white hover:bg-gray-50"
          >
            Clear Filters
          </button>
        )}
          ...
          {activeReport === 'overview' && reportData && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie data={reportData.eventAttendanceData || []} cx="50%" cy="50%" labelLine={false} label={({ name, percent }) => `${name} ${percent ? (percent * 100).toFixed(0) : 0}%`} outerRadius={100} fill="#8884d8" dataKey="value">
                      {(reportData.eventAttendanceData || []).map((entry: any, index: number) => (
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm lg:col-span-2">
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={reportData.paymentHistoryData || []}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="name" stroke="#9ca3af" fontSize={12} />
                    <YAxis stroke="#9ca3af" fontSize={12} />
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    <tr className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <p className="text-sm font-bold text-gray-900">{reportData.organization || 'Your Org'}</p>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <p className="text-sm font-bold text-indigo-600">{reportData.membershipType}</p>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <p className="text-sm font-bold text-gray-900">{reportData.registrationDate}</p>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-bold rounded-full ${reportData.status === 'Active' ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-100 text-gray-800'}`}>{reportData.status}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <p className="text-sm font-bold text-emerald-600">{reportData.renewalDate || 'N/A'}</p>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="inline-flex px-2 py-1 text-xs font-bold rounded-full bg-blue-100 text-blue-800">{reportData.verified}</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
