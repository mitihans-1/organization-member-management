...
const VerifyId = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['verifyId', token],
    queryFn: () => api.get(`/id-cards/verify/${token}`).then(res => res.data),
    retry: false,
  });
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center font-poppins">
        <div className="animate-pulse text-gray-400 font-bold">Verifying ID Card...</div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-poppins">
        <div className="bg-white p-8 rounded-3xl shadow-xl max-w-sm w-full text-center border border-red-100">
          ...
          <h2 className="text-2xl font-black text-gray-900 tracking-tight">Invalid ID Card</h2>
          <p className="text-gray-500 mt-2 text-sm">This QR code does not match any valid ID card in our system. It may be fake or incorrectly scanned.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4 font-poppins py-12">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
        ...
        {/* Status Header */}
        <div className={`p-6 text-center ${data.status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-red-500'}`}>
          ...
          <h2 className="text-white font-black text-2xl tracking-tight uppercase">
            {data.status === 'ACTIVE' ? 'Verified & Active' : `Card ${data.status}`}
          </h2>
        </div>
        ...
        <div className="p-8 flex flex-col items-center relative">
          <div className="w-32 h-32 rounded-full border-4 border-white bg-gray-200 shadow-xl -mt-16 overflow-hidden relative z-10">
            <img 
              src={data.photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(data.name)}&background=e0e7ff&color=3730a3`} 
              alt={data.name} 
          ...
          <div className="text-center mt-6 w-full">
            ...
            <p className="text-indigo-600 font-bold uppercase tracking-widest text-sm mt-1">{data.role}</p>
          </div>

          <div className="w-full mt-8 space-y-4">
            <div className="flex bg-indigo-50 p-4 rounded-xl border border-indigo-100 text-center items-center justify-center gap-1.5">
              <span className="font-black text-indigo-900 text-lg leading-tight">{data.organization}</span>
              <span className="font-bold text-indigo-500 text-lg uppercase tracking-wider">Identification Card</span>
            </div>
            
            <div className="flex gap-4">
              <div className="flex-1 flex flex-col bg-gray-50 p-3 rounded-xl border border-gray-100">
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Card No</span>
                <span className="font-mono text-sm text-gray-900 mt-0.5">{data.cardNumber}</span>
              </div>
